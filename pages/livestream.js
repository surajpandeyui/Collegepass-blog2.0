...
import MyStreamsScreen from '../screens/MyStreamsScreen'

// https://api.collegepass.org/api/v1/events/getLiveSession

import axios from 'axios'
import { APIgetLiveSession, APIgetArchiveStream, APIgetMasterClass, APIgetRecordedTypes } from '../config/API'
...
export async function getStaticProps() {
  ...
  const recordedLiveStreams = await axios.get(APIgetArchiveStream)
  const upcomingMasterclass = await axios.get(APIgetMasterClass)
  const recordedMasterclass = await axios.get(
  ...
  return {
    props: {
      ...
      recordedLiveStreams: recordedLiveStreams.data.data.concat(recordedMasterclass.data.data),
