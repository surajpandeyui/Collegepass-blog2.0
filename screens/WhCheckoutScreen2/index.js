...
import {
  ...
  APIgetLiveSession,
  APIgetLandingPageInfo,
  APIeventRegistration,
  APIstoreUserFromRegistration
  // APIgetAllCities,
} from '../../config/API'
...
const whCheckoutScreen = ({ planType }) => {
  ...
  //   await axios(`${APIgetLandingPageInfo}${email}`)
  //   .then((res)=>{
  ...
  useEffect(() => {
    // console.log("form updated", formDetails)
  }, [formDetails])
  ...
  const prePaymentRegistration = async () => {
    const data = {
      firstName: formDetails.firstName,
      lastName: formDetails?.lastName ? formDetails.lastName : '',
      email: formDetails.email,
      countryCode: formDetails?.countryCode ? formDetails.countryCode : null,
      phoneNumber: formDetails.phoneNumber,
      grade: formDetails?.grade ? formDetails.grade : null,
      city: formDetails?.city ? formDetails.city : null,
      school_name: formDetails?.school ? formDetails.school : '',
      curriculum: formDetails?.curriculum ? formDetails.ciurriculum : '', 
      platform: "WEB"
    }
    // console.log("storing user")
    const result = await axios.post(APIstoreUserFromRegistration, data)
    if (result.data.status) {
      ...
      //   firstName:  formDetails.firstName,
      //   lastName: formDetails?.lastName ? formDetails.lastName : '',
      //   phoneNumber: formDetails?.phoneNumber ? formDetails.phoneNumber : null,
      //   grade:formDetails?.grade ? formDetails.grade : null,
      //   curriculum: formDetails?.curriculum ? formDetails.ciurriculum : '',
      //   // city: '',
      //   parentsNumber: '',
      ...
      // })
      // console.log("checking user type")
      const reply = await checkUserType(formDetails.email)
      return true
      
    }
    else {
      // console.log("retrunging false")
      return false
    }
  }
  ...
  useEffect(() => {
    ...
    if (userStored && userUpdated) {
      // console.log("userStored is true")
      let body = {
  ...
  const payRazorPay = async () => {
    ...
    // {
    //   getUserDetails(formDetails.email)
    // }
    if (user === null) {
      setuserStored(await prePaymentRegistration())
      // console.log("payment reg returned", userStored)
